...
```{r}
set.seed(123)
ind <- rbinom(50, size = 1, prob = 0.03)
x <- (1-ind)*rpois(50, lambda = 7) + ind*20
```
...
```{r}
table(x)/length(x)
ds <- signif(dpois(0:20, lambda = 7), 2)
names(ds) <- 0:20
ds
```
...
That is, observations of 0 and 1 as well as of 13 or larger can be considered as 
